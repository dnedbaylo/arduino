...
// channel vars
// boolean redLED_on[] = {LOW, LOW, LOW, LOW};
...
int tweak1[] = {512, 800, 512, 512}; // 10 bit (0-1024) used for attack, or fine tuning LFO, portamento for noise, duty cycle for pulse,
int tweak2[] = {512, 100, 512, 512}; // 10 bit (0-1024), release, or max amplitude for LFOs, (or randomness)
int phase[] = {0, 0, 0, 0};
...
void loop() {
  ...
  // read pots    // ? period needs to be exponential - little difference in slow half of pot travel
  for (int ch = 0; ch < 4; ch ++) {
    ...
    if (abs(temp - potValue[ch]) > POT_HYSTERESIS) { // only change if pot has moved a bit, removes jitter
      ...
      if (editCh == 0) { // not edit mode, so update frequencies according to pot
  ...
  //  Serial.print(" <- ch 0 : ch 2  -> ");
  //  Serial.println(value[2]);

  // Serial.print(place[3]);
  // Serial.print(" along ");
...
void checkGates() {
  for (int i = 0; i < 3; i ++) {
    ...
    if (gate[i] == true) {
      ...
      bitClear(redLED_on, i); // turns on LED
    }
  }
  if (trigger[2]) {
...
void checkButtons() {
  // buttonPressed holds current button
  ...
  // quickly pressed true at end of short press
  // longPress holds button number, true when button released after HOLD TIME
  ...
  if (temp > LOW1) { // something is pressed
    ...
    if ((newPress) && (millis() < buttonStartTime + HOLD_TIME)) {
      ...
      buttonPressed = 0;
      newPress = false;
    } else if ((newPress) && (millis() >= buttonStartTime + HOLD_TIME)) {
...
/*
  void processButtons() {
  if (longPress > 0) { // let go after long press
    waveform[longPress - 1] ++;
    if (waveform[longPress - 1] >= MAX_WAVEFORMS) {
      waveform[longPress - 1] = 0;
    }
    Serial.print("channel ");
    Serial.print(longPress - 1);
    Serial.print(" : wave: ");
    Serial.println(waveform[longPress - 1]);
    //   bitSet(redLED_on, longPress - 1); // turns on LED
    //   bitSet(redLED_on, longPress + 3); // bits 4-7 remember which LED was turned on by long press, so can turn off at end of button press
    longPress = 0;
  }

  //  if (buttonPressed == 0) { // turn off LED at end of press
  //   for (int i = 0; i <= 3; i ++) {
  //     if (bitRead(redLED_on, i + 4)) { // was it turned on by buttonPressed?
  //        bitClear(redLED_on, i); // if so, kill it
  //       bitClear(redLED_on, i + 4);
  //     }
  //   }
  }
*/
...
void updateOutputs() {
  ...
  //  if ((trigger[2]) && (waveform[3] == EG)) { // 4th channel triggered by 3rd gate
  //    place[3] = 0; // do this before looking at waveforms
  //  }
...
int sine(int i, int phase) {
  int place = i - phase - 255; // 255 to get sine to start at 0
  if (place < 0) {
    place += 1024;
  }
